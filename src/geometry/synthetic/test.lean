...
(B3 : ∀ l ∈ lines, ∀ a b c ∈ l, xor (xor (B a b c) (B b c a)) (B c a b))
 --(B3 : ∀ l ∈ lines, ∀ a b c ∈ l, (B a b c ∧ ¬ B b a c ∧ ¬ B b c a)∨ (¬ B a b c ∧  B b a c ∧ ¬ B b c a)
 --∨ (¬ B a b c ∧ ¬ B b a c ∧  B b c a))
...
theorem PlaneSep {l : set point} (hl : l ∈ lines): ∃ (S1 S2 : set point), S1≠∅ ∧ S2≠∅ ∧
...
theorem LineSep (a : point) (l ∈ lines) : ∃ (S1 S2 : set point), S1≠∅ ∧ S2≠∅ ∧
(∀ ( b c : point), b∈ S1 ∧ c∈ S1 ↔ ¬ (a ∈ lineseg b c))∧
 (∀ ( d e : point), d∈ S1 ∧ e∈ S2 ↔ ¬ (a ∈ lineseg d e))∧
 (∀ ( f g : point ), f∈ S1 ∧ g∈ S2 ↔ a ∈ lineseg f g ):=sorry

/-
theorem trivi  {a : point }{l ∈ lines}:l=l :=
begin

   have hi:= LineSep a l H,
   rcases LineSep a l H with ⟨ S1, S2, hyp, hyp2, hyp3, hyp4, hyp5⟩ ,
   refl,
end
-/
...
class axioms3 extends incidence_geometry :=
...
begin
intro hyp,
have := B1,-- a b a b c d (C4 a b) ,--hyp,
end
